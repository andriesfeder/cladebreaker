...
> **If downloads keep failing**: NCBI throttles aggressive clients with HTTP 503 errors. Lower `--download_forks` (default `4`) to reduce concurrency, and raise `--download_max_retries` if individual chunks fail intermittently.
...
| `--download_chunk_size INT` | Accessions per parallel download job (default: `2000`) |
| `--download_forks INT` | Maximum concurrent NCBI download jobs (default: `4`) |
| `--download_max_retries INT` | Retries per chunk when NCBI throttles a download (default: `5`) |
...
| `--gsi_max_joint_groups INT` | `14` | Skip the joint monophyly probability beyond this many groups |
...
| `gsi/` | The genealogical sorting index keeps its own directory rather than joining `analyze/`: `cladebreaker.gsi.tsv` (per-group gsi and p-value), `.gsi.json` (same plus run settings), `.gsi_mqc.tsv` (MultiQC table), `.gsi_per_tree.tsv` (per-topology values, ensembles only), and `gsi_groups.tsv` (the group assignments used, when generated by `--run_gsi`) |
...
| `test_analyze` | Minimal test of the whole `analyze` decision path against a published worked example. One group there is a clade and one is not, so a single run exercises both branches. Requires no additional parameters. |
