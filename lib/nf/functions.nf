...
def get_schemas() {
    ...
    if (params.workflows[params.wf].containsKey('includes')) {
        ...
        println(params.workflows[params.wf]["includes"])
...
def get_resources(profile, max_memory, max_cpus) {
    ...
    def Map resources = [:]
    resources.MAX_MEMORY = ['conda','standard', 'docker', 'singularity'].contains(profile) ? _get_max_memory(max_memory).GB : (max_memory).GB
...
def _get_max_memory(requested) {
    ...
    //TODO: Added string split here to handle int to string comparison issue.
    if (available < requested.split("\\.")[0].toInteger()) {
