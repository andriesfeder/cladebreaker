# nf-core/cladebreaker: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/cladebreaker/usage](https://nf-co.re/cladebreaker/usage)
...
```console
nextflow run nf-core/cladebreaker --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/cladebreaker
```
...
First, go to the [nf-core/cladebreaker releases page](https://github.com/nf-core/cladebreaker/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
